...
* 
*/
class ControllerCatalogEmailVerification extends Controller
{
    private $error = array();
    private $version;
    ...
    public function index () {        
        ...
        //load language
        $this->load->language('catalog/emailverification');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link('catalog/emailverification', 'store_id='.$this->request->post['store_id'].'&token=' . $token, 'SSL'));
        }
        ...
        $data['breadcrumbs'][]              = array(
            ...
            'href'                  => $this->url->link('catalog/emailverification', 'token=' . $token, 'SSL'),
        );
        ...
        $data['action']                 = $this->url->link('catalog/emailverification', 'token=' . $token, 'SSL');
        $data['cancel']                 = $this->url->link('common/dashboard', 'token=' . $token, 'SSL');
        ...
        $this->response->setOutput($this->load->view('catalog/emailverification'.'.tpl', $data));
    }
    ...
    public function install() {
        $this->load->model('catalog/emailverification');
        $this->model_catalog_emailverification->install();
    }
     public function uninstall() {
        $this->load->model('catalog/emailverification');
        $this->model_catalog_emailverification->uninstall();
    }
    
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'catalog/emailverification')) {
