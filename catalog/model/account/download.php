...
class ModelAccountDownload extends Model {
	public function getDownload($download_id) {
        ...
        //Momen has removed these lines because we no more are using the download page
        //so we need this method to work custom to th product usage.
...
//		}
        ...
        $query = $this->db->query("SELECT filename, mask FROM `" . DB_PREFIX . "download` WHERE  download_id = '" . (int)$download_id . "'");
    ...
    public function getDownloadId($product_id) {
        $query = $this->db->query("SELECT download_id FROM `" . DB_PREFIX . "product_to_download`  where product_id = '" . (int)$product_id . "'");
