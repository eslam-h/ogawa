...
class ControllerProductProduct extends Controller {
    ...
    public function index() {
        ...
        if ($product_info) {
            ...
            $data['button_request_product'] = $this->language->get('button_request_product');
            $data['contact_form'] = $this->url->link('information/contact');

            $this->load->model('account/download');

            $download_id = $this->model_account_download->getDownloadId($product_info['product_id']);

            if(isset($download_id))
            {
                $data['text_download'] = $this->language->get('text_download');
                $data['download_link'] = $this->url->link('account/download/download', 'download_id=' . $download_id, true);
            }
        ...
        $data['salescombopgeoffers'] = $this->model_offers_salescombopge->getOfferByProductId($product_id);
            ...
            foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
                ...
                foreach ($option['product_option_value'] as $option_value) {
                    if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                        ...
                        $product_option_value_data[] = array(
                            ...
                            'english_name'            => $option_value['english_name'],
                            'option_value_quantity'   => $option_value['quantity'],
                        );
                    }
                }
                ...
                {
                    $data['selected_option'] = $product_option_value_data[0]['english_name'];
                    ...
                    foreach ($product_option_value_data as $option_value)
                    {
                        ...
                        {
                            if (strpos($result['image'], '-'.$option_value['english_name']) !== false)
                            {
                                if($check == 1)
                                {
                                    $check = 2;
                                    $data['popup'] = $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
                                    $data['thumb'] = $this->model_tool_image->resize($result['image'], $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
                                }
                        ...
                        $data['images'][$option_value['english_name']] = $images;
                    }
                }
            }

//            if ($data['images'][$data['selected_option'][0]]) {
//

            if($check != 2)
            {
                $data['selected_option'] = 'Cannot be an image';
                $data['images'][$data['selected_option']] = '';
                $data['popup'] = '';
                $data['thumb'] = '';
            }
            ...
            $data['features'] = $this->model_catalog_product->getProductFeatures($this->request->get['product_id']);
